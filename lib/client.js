  ...
  document.addEventListener('click', function (event) {
    ...
    if (body.hasAttribute('hidden')) {
      ...
      root.removeAttribute('data-open')
		...
		*/
		function buildCloneHtml(nodeKeys) {
			const wanted = new Set(nodeKeys);
			const seats = Array.from(document.querySelectorAll("[data-chat-flow-key]"));
			const parts = [];
		...
		function buildDocument(pin, t, stylesheets, cloneHtml) {
  ...
  ${TOGGLE_SCRIPT}
		...
		*/
		function openPinWindow(pin, t) {
			const html = buildDocument(pin, t, Array.from(document.querySelectorAll("link[rel=\"stylesheet\"]")).map((link) => link.getAttribute("href")).filter((href) => typeof href === "string"), buildCloneHtml(pin.nodeKeys));
