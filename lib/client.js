		...
		*/
		...
		function renderBlock(block, t) {
			switch (block.kind) {
				...
				case "tool-call": return `<details class="block tool"><summary class="tool-summary">${escapeHtml(t("window.toolCall"))} · ${escapeHtml(block.name)}</summary><pre class="tool-args">${escapeHtml(block.argsRaw)}</pre></details>`;
				case "image": return `<section class="block image">${escapeHtml(t("window.image"))}</section>`;
				case "other": return `<details class="block other"><summary class="other-summary">${escapeHtml(t("window.other"))}</summary><pre>${escapeHtml(JSON.stringify(block.block, null, 2))}</pre></details>`;
			}
		}
