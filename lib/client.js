...
`;
		...
		*  summaries, and wire disclosure-row toggling for the cloned DOM. */
		function buildToggleScript(firstLines) {
  ...
  document.addEventListener('click', function (event) {
    ...
    if (body.hasAttribute('hidden')) {
      ...
      root.removeAttribute('data-open')
		...
		*/
		function expandDisclosureRows(seats) {
			const expandedRows = [];
			for (const seat of seats) for (const row of Array.from(seat.querySelectorAll("[data-disclosure-row]"))) {
		...
		function collapseDisclosureRows(rows) {
			for (const row of rows) if (row.isConnected) row.click();
		}
		/** Build the full popup document for one pinned turn. */
		function buildDocument(pin, t, stylesheets, cloneHtml, pluginCss) {
  ...
  ${buildToggleScript(pin.reasoningFirstLines)}
		...
		*/
		async function openPinWindow(pin, t) {
			...
			if (expandedRows.length > 0) await new Promise((resolve) => setTimeout(resolve, 250));
