...
`;
		...
		*  summaries, and wire disclosure-row toggling for the cloned DOM. */
		function buildToggleScript(firstLines, leadings) {
		...
		*/
		function cloneSeats(seats) {
			...
			for (const seat of seats) {
				Array.from(seat.querySelectorAll("button")).forEach((button, index) => {
...
<body${dark ? " data-ds-dark-theme" : ""}>
  ...
  ${buildToggleScript(pin.reasoningFirstLines, leadings)}
		...
		*/
		...
		function findStampedButton(seat, btnId) {
			return seat.querySelector(`[data-pin-btn="${escapeAttr(btnId)}"]`);
		}
		/** Handle a forwarded button click from a pin popup. */
		function handlePinWindowMessage(event) {
			const data = event.data;
			if (data === null || data.dshPinWindow !== true) return;
			if (typeof data.seatKey !== "string" || typeof data.btnId !== "string") return;
			if (event.origin !== window.location.origin) return;
			const seat = document.querySelector(`[data-chat-flow-key="${escapeAttr(data.seatKey)}"]`);
			if (seat === null) return;
		...
		*/
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				return () => {
					window.removeEventListener("message", handlePinWindowMessage);
				};
