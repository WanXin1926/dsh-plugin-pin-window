...
DeepSeek Harness 网页版插件：在每条助手消息下方的操作条里新增一个 **固定消息（Pin）** 按钮。点击后打开一个以只读展示为主的新窗口，只展示被选中的那条消息（及其所在的完整 turn）。复制代码、打开文件、Inspect 等操作会安全地转发回原页面执行。
...
- 点击后弹出新窗口：
  ...
  - 跟随当前深色 / 浅色主题
- 交互：
  - 代码块复制按钮在 popup 本地直接复制，不切回原页面
  - 文件路径链接 / Inspect 等按钮会转发回原页面执行（原页面刷新或切到别的会话后仍会尝试自动切回目标会话）
...
- 转发类按钮（打开文件 / Inspect 等）依赖原页面还能定位到对应消息：原页面刷新或切换会话会自动切回目标会话；但如果该会话已不可见（例如被窗口截断、删除），这类转发会静默失败。代码块复制不受影响。
- 安全面：popup 通过 `window.opener.postMessage` 转发点击，消息带 `dshPinWindow` 标记并校验 `origin`。
...
- popup 内的复制按钮本地写入剪贴板；其他按钮按 `b<n>` 序号转发回原页面，原页面在转发时若发现会话不对会先 `sessions.open` 切回目标会话
